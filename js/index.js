...
function GET_USER_ID_AND_ITEM_COMPLETE(name,content){
    let ID,itemID;
    fetch(`${BASE_URL}api/users`)
        .then(response=>response.json())
        .then(data=>{
        ...
        .then(([ID,itemID])=>{ITEM_COMPLETE(ID,itemID)});
}

//해당되는 ID와 todoitem의 ID를 가져와 처리한다.
function ITEM_COMPLETE(ID,itemID){
    fetch(`${BASE_URL}api/users/${ID}/items/${itemID}/toggle`,
        {
            method:"PUT",
        })
        .then(response=>response.json())
...
// 체크박스 버튼 클릭시 해당 TODOLIST의 ITEM_COMPLETE함수 실행
function GET_USER_ID_AND_MODIFY_TODOLIST(name,Before_Content,After_Content){
    let ID,itemID;
    fetch(`${BASE_URL}api/users`)
        .then(response=>response.json())
        .then(data=>{
            ...
            console.log(ID,itemID,After_Content)
            return [ID,itemID,After_Content];
        })
        .then(([ID,itemID,After_Content])=>{ITEM_COMPLETE(ID,itemID,After_Content)});
}

//해당되는 ID와 todoitem의 ID를 가져와 처리한다.
function ITEM_COMPLETE(ID,itemID,After_Content){
    fetch(`${BASE_URL}api/users/${ID}/items/${itemID}`,
        {
            method:"PUT",
            headers: {
...
function init_Element(value,item,cause_of_add) {
    ...
    document.querySelectorAll('.toggle').forEach($el => $el.addEventListener('click', function (event) {
        if ($el.checked === true) {
            ...
            GET_USER_ID_AND_ITEM_COMPLETE(document.querySelector('#user-list > .active').innerHTML, value,input_tag.value)
        } else {
    ...
    document.querySelectorAll('.label').forEach($el => $el.addEventListener('dblclick',function(event){
        ...
        input_tag.addEventListener('keydown', function (e) {
            if (e.key === 'Enter' && /[\S]/gi.test(input_tag.value)==true && check_overlap(input_tag.value) == true) {
                ...
                GET_USER_ID_AND_MODIFY_TODOLIST(document.querySelector('#user-list > .active').innerHTML, event.target.closest('div').children[1].innerText,input_tag.value)
